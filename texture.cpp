...
//     texture_t *thisTexture;       // the texture
	...
	w = width;
     ...
     scaledHeight = (long)pow((double)2, i-1);
     ...
     scaledWidth = (long)pow((double)2, i-1);
...
void CTexture::Draw(CTexture texture, GLfloat x, GLfloat y, GLfloat width, GLfloat height)
{
	glPushMatrix();
	glLoadIdentity();
	glBindTexture(GL_TEXTURE_2D, texture.texID);
	glBegin(GL_QUADS);
		glTexCoord2f(1.0f,1.0f); glVertex3f(  x+width, y+height, 0.0f);	// Top Right
		glTexCoord2f(0.0f,1.0f); glVertex3f(  x      , y+height, 0.0f);	// Top Left
		glTexCoord2f(0.0f,0.0f); glVertex3f(  x      , y       , 0.0f);	// Bottom Left
		glTexCoord2f(1.0f,0.0f); glVertex3f(  x+width, y       , 0.0f);	// Bottom Right
	glEnd();
